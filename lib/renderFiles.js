...
class RenderFiles {
    ...
    _renderPage(template, path, model, modelPath) {
        let extension = this._extensionRegex.exec(template)[1];
        ...
        this._env.render(this._templatesFolder + '/' + template, model, (err, res) => {
            ...
            /*
            * See if transistor must render a folder or a file
            */

            if(this._extensionRegex.exec(path)[1]) {
                //is a file, because the path contains a extension

                fs.outputFile(location, res);
            } else {
                fs.outputFile(location + '/index.' + extension, res);
            }
        });
    }
                ...
                if(typeof route.model == 'string') {
                    if(route.model.includes('.*')){
                        ...
                        for (var key in _pseudoModel) {
                            _temporaryModel = _.merge({}, this._engine, _pseudoModel[key]);
